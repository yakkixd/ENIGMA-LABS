...
const Skills = () => {
    const skillCategories = [
        ...
        },
        {
            ...
            skills: ["NumPy", "Matplotlib", "Pandas", "Geopandas"],
            color: "violet"
        },
        {
            title: "Databases",
    ...
    return (
        <section id="armory" className="relative bg-black border-t border-purple-500/20">
            ...
            <div className="section-container relative z-10">
                ...
                {/* Skill Categories Grid for quick scanning */}
                <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6 max-w-6xl mx-auto opacity-80 hover:opacity-100 transition-opacity duration-500">
                    {skillCategories.map((category, index) => (
                        <ScrollReveal key={index} delay={index * 0.05} className="h-full">
                            <div className="group relative h-full hover:-translate-y-1 transition-transform duration-300 ease-out">
                                {/* Card */}
                                <div className="relative h-full bg-purple-900/5 border border-purple-500/10 rounded-xl p-6 overflow-hidden transition-colors duration-300 group-hover:bg-purple-900/10">
                                    {/* Top gradient line */}
                                    <div className={`absolute top-0 left-0 right-0 h-0.5 bg-gradient-to-r ${category.color === 'purple' ? 'from-purple-600 to-violet-600' : 'from-violet-600 to-purple-600'}`}></div>

                                    <div className="flex items-center gap-4 mb-4">
                                        <div className="w-10 h-10 text-purple-400 group-hover:text-purple-300 transition-colors">
                                            {category.icon}
                                        </div>
                                        <h3 className="text-lg font-bold tracking-tight text-white group-hover:text-purple-200 transition-colors">
                                            {category.title}
                                        </h3>
                                    </div>
                                    ...
                                    <div className="flex flex-wrap gap-2">
                                        {category.skills.map((skill, skillIndex) => (
                                            <span
                                                key={skillIndex}
                                                className="px-2 py-1 bg-purple-500/10 text-xs font-mono text-purple-300 rounded border border-purple-500/20"
                                            >
                                                {skill}
                                            </span>
                                        ))}
